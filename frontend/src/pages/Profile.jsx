        ...
        {isEditing ? (
          <form onSubmit={handleUpdate} className="mt-10 animate-in slide-in-from-bottom-8 duration-500">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 p-6 rounded-[2rem] shadow-sm dark:shadow-none backdrop-blur-xl transition-colors duration-500">
              ...
              <button type="submit" className="md:col-span-2 w-full bg-gradient-to-r from-[#1d9bf0] to-purple-600 text-white font-black py-4 rounded-2xl hover:opacity-90 transition-all active:scale-[0.98] cursor-pointer text-lg mt-2 shadow-lg">
                Save Profile ✨
              </button>
            </div>
          </form>
...
const TabItem = ({ active, label, onClick }) => (
  <button onClick={onClick} className={`pb-3 px-2 text-xs md:text-sm font-black uppercase tracking-widest relative transition cursor-pointer ${active ? 'text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-400'}`}>
    {label}
